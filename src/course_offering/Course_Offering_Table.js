...
import { deleteCourse_Offering, getCourse_Offerings } from '../action/course_offering.action';
const style = {
    position: 'absolute',
    top: '50%',
    left: '50%',
...
const headCells = [
  ...
  {
    id: 'course_break_down_id',
    numeric: true,
    disablePadding: false,
    label: 'Course Breakdown Id',
  },
...
export default function Course_Offering_Table() {
  ...
  const courseOfferingRows =  useSelector((state) => state.course_offeringReducer);

  const rows = courseOfferingRows;
  
  const handleDelete = (id) => {
    dispatch(deleteCourse_Offering(id));
  }
  return (
    <Box sx={{ width: '90%' }}>
