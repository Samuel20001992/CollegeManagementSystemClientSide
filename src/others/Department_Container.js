...
import Department_Table from '../department/Department_Table';
import { useDispatch } from 'react-redux';
import { getOneDepartment } from '../action/department.action';
function Department_Container() {
  ...
  const handleSearch = () =>{
    dispatch(getOneDepartment(searchValue)); 
}
    ...
    <Box sx={{ width: '100%', typography: 'body1' }}>
      ...
      <Grid container >
        ...
        <Grid item xs={10} md={10} sm={10} style={{float:'left'}}>
          <Typography variant='h6' style={{float:'left',  color:'gray', marginTop:'10px', marginLeft:'20px'}}>Department</Typography>
          ...
          <TabContext value={value} >
            <Box style={{marginTop:'30px'}} >
              <TabList onChange={handleChange} aria-label="lab API tabs example">
                <Tab label="Add Department" value="1" />
                <Tab label="Department List" value="2" />
              </TabList>
            </Box>
            <TabPanel value="1" style={{backgroundColor:'#E7E9EB', height:'470px'}}>
              <div >
                <Add_Department change={change} />      
              </div>
            </TabPanel>
...
export default Department_Container
