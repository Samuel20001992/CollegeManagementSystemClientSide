...
import { getOneSemesterGrade } from '../action/semester_grade.action';
import Add_Semester_Grade from '../semester_grade/Add_Semester_Grade';
function Semester_Grade_Container() {
 ...
 const handleSearch = () =>{
   dispatch(getOneSemesterGrade(searchValue)); 
}
    ...
    <Box sx={{ width: '100%', typography: 'body1' }}>
      ...
      <Grid container >
        ...
        <Grid item xs={10} md={10} sm={10} style={{float:'left'}}>
          <Typography variant='h6' style={{float:'left',  color:'gray', marginTop:'10px', marginLeft:'20px'}}> Grade</Typography>
          ...
          <TabContext value={value} >
            <Box style={{marginTop:'30px'}} >
              <TabList onChange={handleChange} aria-label="lab API tabs example">
                <Tab label="Semester Grade " value="1" />
                {/* <Tab label="Add List" value="2" /> */}
              </TabList>
            </Box>
            <TabPanel value="1" style={{backgroundColor:'#E7E9EB', height:'550px'}}>
              <div >
                <Add_Semester_Grade change={change} />      
              </div>
            </TabPanel>
...
export default Semester_Grade_Container
